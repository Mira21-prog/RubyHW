...
p 'm = [1, 2, 3, 4, 5, 6, 7]'
m = [1, 2, 3, 4, 5, 6, 7]
p m.select.with_index { |_word, idx| idx.even? } + m.select.with_index { |_word, idx| idx.odd? }
p '-----------------------------------------------'

p '2. Дан целочисленный массив. Необходимо вывести вначале его элементы с нечетными индексами, а затем - четными.'
p 'm = [1, 2, 3, 4, 5, 6, 7]'
m = [1, 2, 3, 4, 5, 6, 7]
p m.select.with_index { |_word, idx| idx.odd? } + m.select.with_index { |_word, idx| idx.even? }
p '-----------------------------------------------'
...
def some_factorial(n)
  p n.even? ? (1..n).select { |n| n.even? }.inject(:*) : (1..n).select { |n| n.odd? }.inject(:*)
end
some_factorial(n)
p '-----------------------------------------------'
